...
class BM25(object):
    ...
    def __init__(self, corpus, idf,load_status = 0):
        """
        Parameters
        ----------
        corpus : list of list of str
    ...
    def initialize(self):
        ...
        if self.idf_status == 0:
            
            for word, freq in iteritems(self.df):
                self.idf[word] = math.log(self.corpus_size - freq + 0.5) - math.log(freq + 0.5)
                
                
        elif self.idf_status == 1:
            ...
            with open("../support/trec_idf.txt", "w", encoding='utf-8') as record_object:
                for word, freq in iteritems(self.df):
                    self.idf[word] = math.log(self.corpus_size - freq + 0.5) - math.log(freq + 0.5)
                for idf_key in self.idf.keys():
                    record_object.writelines(idf_key + " " + str(self.idf[idf_key]))
                    record_object.writelines("\n")
        elif self.idf_status == 2:
            print("enter this once")
            with open("../support/trec_idf.txt", "r", encoding='utf-8') as record_object:
                lines = list(record_object.readlines())
                for line in lines:
                    tmp_data = line.replace('\n','').split(" ")
                    self.idf[tmp_data[0]] = float(tmp_data[1])
        elif self.idf_status == 3:
            pass
    ...
    添加vn_model对文档长度的调整
    '''
    def get_score_for_vn(self, document, index, average_idf):
        """Computes BM25 score of given `document` in relation to item of corpus selected by `index`.
        ...
        for word in document:
            if word not in self.f[index]:
                continue
            idf = self.idf[word] if self.idf[word] >= 0 else EPSILON * average_idf
            ad_len = self.doc_len[index] / (self.doc_scope[index] + 0.1)
            ad_len = ad_len if ad_len > 0  else 0
            score += (idf * self.f[index][word] * (PARAM_K1 + 1)
                      / (self.f[index][word] + PARAM_K1 * ( (1 - PARAM_B) * ad_len + PARAM_B * self.doc_len[index] / self.avgdl)))
            '''
            score += (idf * self.f[index][word] * (PARAM_K1 + 1)
                      / (self.f[index][word] + PARAM_K1 * ( (1 - PARAM_B) * ad_len + PARAM_B * 1 / self.avgdl)))
            '''
        return score

    def get_scores_for_vn(self, document, average_idf):
        """Computes and returns BM25 scores of given `document` in relation to
        every item in corpus.
        ...
        scores = []
        for index in xrange(self.corpus_size):
            score = self.get_score_for_vn(document, index, average_idf)
            scores.append(score)
        return scores
