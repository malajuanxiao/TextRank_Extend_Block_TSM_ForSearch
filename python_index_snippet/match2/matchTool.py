...
class Match(object):
    ...
    def __init__(self, corpus,idf, w2v, WMD_have = 0):
    ...
    #bm25公式
    def bmscore(self, que ,doc, slice_index ,index, avgdl, average_idf):
        score = 0
        for word in que:
            if word not in doc:
                continue
            ...
            score += (idf * self.f[index][word] * (PARAM_K1 + 1)
                      / (self.f[index][word] + PARAM_K1 * (1 - PARAM_B + PARAM_B * len(doc) / avgdl)))
            """
            score += (idf * self.f[index][word] * (PARAM_K1 + 1)
                      / (self.f[index][word] + PARAM_K1 * (1 - PARAM_B + PARAM_B * len(doc) / self.avgdl)))
            """
        return score
    
    
    #获取bm25  选取最大
    def bm25Sim(self, qu_doc, doc_slice, doc_index,limit_ratio,filter_score):
        sim_score_snippets = []
        doc_sni_length = len(doc_slice)
        #print(doc_sni_length)
        if doc_sni_length != 0:
        ...
        for slice_index,sni_item2 in enumerate(doc_slice):
            bm_score = self.bmscore(qu_doc,sni_item2,slice_index,doc_index, avgdl, self.average_idf) #*self.Swei(sni_item1,sni_item2)
            
            
            if bm_score >= filter_score:
                #print(slice_index,bm_score)
                sim_score_snippets.append((slice_index,bm_score))
                relev_sni_length += 1
                #print(relev_sni_length,bm_score)
                
        
        rel_snippet_ratio = relev_sni_length*1.0/(doc_sni_length + 0.0000001)
        rel_snippet_ratio = rel_snippet_ratio if rel_snippet_ratio < limit_ratio else limit_ratio
        #print(rel_snippet_ratio,'\n',len(sim_score_snippets))
    ...
    def bm25Sim(self, qu_slice, doc_slice, doc_index,limit_ratio,filter_score):
        ...
        for slice_index,sni_item2 in enumerate(doc_slice):
            ...
            for sni_item1 in qu_slice:
                bm_score = self.bmscore(sni_item1,sni_item2,slice_index,doc_index, self.average_idf) #*self.Swei(sni_item1,sni_item2)
            ...
            if sni_select_flag == 1:
                sim_score_snippets.append((slice_index,nmax/que_sni_length))
                relev_sni_length += 1
    ...
    def getwordsim1(self,all_doc_slices,all_que_slices,rel_docs,docs_have,file_index,ing_fun):
        ...
        for idx,qu in enumerate(all_que_slices):
            ...
            for doc_index in doc_have:
                sim_score_snippets, rel_snippet_ratio = self.wordSim1(qu,all_doc_slices[doc_index],0.3,0.2)
                #print(sim_score_snippets, rel_snippet_ratio)
    ...
    def getsimbm25(self,all_doc_slices,all_que_slices,rel_docs,docs_have,file_index,ing_fun):
        ...
        for idx,qu in enumerate(all_que_slices):
            ...
            for doc_index in doc_have:
                sim_score_snippets, rel_snippet_ratio = self.bm25Sim(qu,all_doc_slices[doc_index],doc_index,0.55,9.0)
                '''
                ...
                #print(sim_score_snippets, rel_snippet_ratio)
                score = self.integScore(sim_score_snippets, rel_snippet_ratio,ing_fun)
                bm25_score_list.append((str(file_index[doc_index]),score))
                
                #print(idx,str(file_index[doc_index]),score)
            
            
            bm25_score_list.sort(key=lambda x:x[1],reverse=True)
            #print(len(bm25_score_list))
            score_list.append(bm25_score_list)
    ...
    def getsimwmd(self,all_doc_slices,all_que_slices,rel_docs,docs_have,file_index,ing_fun):
        ...
        for idx,qu in enumerate(all_que_slices):
            ...
            for doc_index in doc_have:
                sim_score_snippets, rel_snippet_ratio = self.wmdCost(qu,all_doc_slices[doc_index],0.6,3.5)
                #print(sim_score_snippets, rel_snippet_ratio)
                score = self.integScore(sim_score_snippets, rel_snippet_ratio,ing_fun)
                wordsim1_score_list.append((str(file_index[doc_index]),score))
                #print(idx,str(file_index[doc_index]),score)
    ...
    def get_score(self, document, index, average_idf):
        ...
        for word in document:
            ...
            '''
            score += (idf * self.f[index][word] * (PARAM_K1 + 1)
                      / (self.f[index][word] + PARAM_K1 * (1 - PARAM_B + PARAM_B * self.doc_len[index] / 20)))
            '''
        return score
    ...
    #原始bm25
    def get_score_acc(self, document, index, average_idf):
        """Computes BM25 score of given `document` in relation to item of corpus selected by `index`.
    ...
    def ewGen_occ(self,que_doc,corpus_words,all_doc_slices,minconf):
        ...
        for qu_word in que_doc:
            if (qu_word in self.bmidf  and self.bmidf[qu_word] <= self.average_idf * 0.5) or qu_word not in self.bmidf:
                print(qu_word +" pass")
                continue
                ...
                for doc_slices in all_doc_slices:
                    for doc_slice in doc_slices:
                        ...
                        if qu_word in doc_slice and word in doc_slice:
                            occ_oc += 1
                #conf = occ_oc*1.0/(occ_qu + occ_ex + 0.000000001)
    ...
    def ewGen_occ_trec(self,que_doc,corpus_words,documents,minconf,minsupp):
        doc_sum = len(documents)
        if doc_sum <= 15:
            return {}
        print(len(documents))
        extend_word = {}
        for qu_word in que_doc:
            if (qu_word in self.bmidf  and self.bmidf[qu_word] <= self.average_idf*4) or qu_word not in self.bmidf:
                print(qu_word +" pass")
                continue
            for word in corpus_words:
                if (word in que_doc) or (word not in self.bmidf) or (self.bmidf[word] <= self.average_idf*4 ) or len(word)<5:
                    continue
                #ex_flag = 0
                #查询项,拓展项,共现次数
                occ_qu = 0
                ...
                for doc in documents:
                    ...
                    if word in doc:
                        occ_ex += 1
                    if qu_word in doc and word in doc:
                        occ_oc += 1
                
                if occ_qu >= doc_sum*0.2:
                    print(qu_word +" pass")
                    break
                ...
                supp = occ_oc*1.0/(occ_qu + occ_ex - occ_oc  - 0.000000001)
                #supp = occ_oc
                
                if conf > minconf and supp > minsupp and occ_oc > doc_sum*0.1 and occ_oc >= 5:
                #if conf > minconf:
                #if conf > minconf and and occ_ex >= 20:
                    re_value = extend_word.setdefault(word, 0)
                    if conf > re_value:
                        extend_word[word] = conf
                    print(qu_word," - ",word,":",conf ,"..", occ_qu," ",occ_ex," ",occ_oc)
        #print(exword_list)
        #que_doc.extend(exword_list)
        #que_doc = list(set(que_doc))
